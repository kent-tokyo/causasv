...
| チェーン | 64 | `approx` シード付き直列（2k、u64 バックエンド） | 2.04 ms |
| チェーン | 65 | `approx` シード付き直列（2k、large バックエンド） | 2.86 ms |
| チェーン | 128 | `approx` シード付き直列（2k、large バックエンド） | 5.57 ms |
| チェーン | 256 | `approx` シード付き直列（2k、large バックエンド） | 12.4 ms |

n=64→65 の行は連合表現の切り替え境界です（上記「n の上限が実際にどこにかかるか」参照）：サンプルあたりのコストは滑らかに増加し（境界自体は約40%増、以降はほぼ n に線形）、n=65 で不連続な崖はありません — `u64` の代わりに小さな `Box<[u64]>` キーをハッシュ・確保するコストが加わるだけです。並列・adaptive・batched・非木形状での測定値は [docs/benchmarks.md](docs/benchmarks.md) を参照してください。
