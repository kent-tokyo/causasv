...
実験的 — v0.8.8。v1.0 以前に公開 API が変更される可能性があります。
...
| チェーン | 64 | `approx` シード付き直列（2k、u64 バックエンド） | 2.24 ms |
| チェーン | 65 | `approx` シード付き直列（2k、large バックエンド） | 3.07 ms |
| チェーン | 128 | `approx` シード付き直列（2k、large バックエンド） | 6.24 ms |
| チェーン | 256 | `approx` シード付き直列（2k、large バックエンド） | 15.6 ms |

n=64→65 の行は連合表現の切り替え境界です（上記「n の上限が実際にどこにかかるか」参照）：サンプルあたりのコストは滑らかに増加し（境界自体は約37%増、以降はほぼ n に線形）、n=65 で不連続な崖はありません — 連合バッファ自体はサンプル間で再利用され再確保されないため、これは `u64` の代わりに `&[u64]` スライスをキャッシュ参照のたびにハッシュするコストです。batched 系はこの境界でより大きな段差（チェーンで約+130%）を払います — これは n > 64 の連合キャッシュが呼び出し全体で永続化されずラウンドごとにスコープされる、意図的な設計判断によるものです。並列・adaptive・batched・非木形状での測定値と batched のトレードオフの詳細は [docs/benchmarks.md](docs/benchmarks.md) と [docs/correctness.md](docs/correctness.md#large-dag-approximate-paths-n--64) を参照してください。
