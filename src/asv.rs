...
impl AsvExplainer {
    ...
    ///
    /// For n ≤ 64, a sample's coalition is a plain `u64` bitmask. For n > 64,
    /// it is instead a growable word-vector bitset (see `crate::coalition`)
    /// with a bounded coalition→value cache — this is an internal
    /// implementation detail, not a caller-visible difference. In practice
    /// the limiting factors for large DAGs are `n_samples`, the cost of
    /// `value_fn`, and memory budget, not `n` itself.
    ...
    ///   else `approximate`
    /// - n > 63: `approximate` — the exact/sparse methods above all require n ≤ 63
    ///   (u64 bitmask); `approximate` itself has no such limit (see its own doc comment)
    ///
        ...
        F: Fn(&[NodeId]) -> Result<f64, CausasvError>,
    {
        ...
        if n <= 8 {
            ...
            if estimate_sparse_feasible(&self.dag, &self.parents_mask, SPARSE_PREFLIGHT_BUDGET) {
                ...
                let fallback_msg = match self.exact_dag_sparse_with_config(|c| vfn(c), &sparse_cfg)
                {
                    Ok(mut r) => {
                        r.method_used = Some("exact_dag_sparse");
                        return Ok(r);
                    }
                    Err(CausasvError::InvalidConfig(msg)) | Err(CausasvError::Overflow(msg)) => msg,
                    Err(e) => return Err(e),
                };
                // exact_dag_sparse failed — try uniform sparse adaptive, then IS adaptive.
                match self.approximate_uniform_sparse_adaptive(|c| vfn(c), config) {
                    Ok(mut r) => {
                        r.fallback_from = Some("exact_dag_sparse".to_string());
                        r.fallback_reason = Some(fallback_msg);
                        r.method_used = Some("uniform_sparse_adaptive");
                        Ok(r)
                    }
                    Err(CausasvError::Overflow(msg)) => {
                        let mut r = self.approximate_adaptive(|c| vfn(c), config)?;
                        r.fallback_from = Some("exact_dag_sparse".to_string());
                        r.fallback_reason = Some(msg);
                        r.method_used = Some("approx_adaptive");
                        Ok(r)
                    }
                    Err(e) => Err(e),
                ...
                // Preflight exceeded 250k budget — try uniform sparse adaptive, then IS adaptive.
                match self.approximate_uniform_sparse_adaptive(|c| vfn(c), config) {
                    Ok(mut r) => {
                        r.method_used = Some("uniform_sparse_adaptive");
                        Ok(r)
                    ...
                    Err(CausasvError::Overflow(msg)) => {
                        let mut r = self.approximate_adaptive(|c| vfn(c), config)?;
                        r.fallback_from = Some("uniform_sparse_adaptive".to_string());
                        r.fallback_reason = Some(msg);
                        r.method_used = Some("approx_adaptive");
                        Ok(r)
                    }
